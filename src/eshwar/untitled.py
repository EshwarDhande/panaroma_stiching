...
def load_and_resize(image_path, width, height):
    ...
    img = cv2.resize(img, (width, height))  # Resize to specified dimensions
    return img  # Return the resized image

# Function to display an image
def display_image(img, width=0, height=0, show_axis=False, img_title=''):
...
def obtain_src_dst_points(image1, image2):
    ...
    keypoints1, descriptors1 = extract_keypoints_descriptors(gray_image1)  # Get keypoints and descriptors
    keypoints2, descriptors2 = extract_keypoints_descriptors(gray_image2)  # Get keypoints and descriptors
...
def estimate_homography_with_ransac(src_pts, dst_pts, iterations=1000, threshold=0.5):
    ...
    for _ in range(iterations):  # Iterate for the specified number of iterations
        ...
        H_candidate = calculate_homography(src_pts[sampled_indices], dst_pts[sampled_indices])  # Compute the homography matrix from selected points
...
def image_alignment(image, H, scale_factor):
...
def crop_background_from_image(image):
...
def transform_images(img1, img2, H, focus=2, blend=True, scale_factor=4, blend_region=5):
    ...
    if focus == 1:  # Wrap the second image into the first image
        transformed_img, height_, width_ = image_alignment(img2, H, scale_factor)  # Align the second image
        ...
        transformed_img, height_, width_ = image_alignment(img1, inverse_H, scale_factor)  # Align the first image
        result_image = transformed_img.copy()  # Copy the transformed image
    ...
    return crop_background_from_image(result_image)  # Return the final image after removing black pixels

# opening the images
image1 = load_and_resize('Images/I1/STA_0031.JPG', 600, 400)
image2 = load_and_resize('Images/I1/STB_0032.JPG', 600, 400)
image3 = load_and_resize('Images/I1/STC_0033.JPG', 600, 400)
image4 = load_and_resize('Images/I1/STD_0034.JPG', 600, 400)
image5 = load_and_resize('Images/I1/STE_0035.JPG', 600, 400)
image6 = load_and_resize('Images/I1/STF_0036.JPG', 600, 400)

# List of images to stitch in sequence (originally in BGR format)
images = [image1, image2, image3, image4, image5, image6]
logging.info(f'Loaded {len(images)} images for stitching')

# Initialize the aligned image with the first image converted to RGB
aligned_image = BGR2RGB(images[0])
logging.info('Initialized aligned_image with the first image')      

# Loop through each image in sequence, stitching each one to the previously aligned result
for i in range(1, len(images)):
    # Convert the current image to RGB to match aligned_image's format
    current_image = BGR2RGB(images[i])
    logging.info(f'Stitching image {i +1} to the aligned image')

    # Obtain source and destination points for feature matching
    src_pts, dst_pts, matching_result = obtain_src_dst_points(aligned_image, current_image)
    logging.info('Obtained source and destination points for feature matching')
    display_image(matching_result, width=20, height=10)
    logging.info('Calulated homography matrix')

    # Estimate homography with RANSAC
    best_H = estimate_homography_with_ransac(src_pts, dst_pts, iterations=10000, threshold=5)
    logging.info('Estimated homography with RANSAC')
    
    logging.info('Aligning the images')
    # Transform and blend images based on the homography
    aligned_image = transform_images(aligned_image, current_image, best_H, focus=2, blend=True)
    logging.info('Transformed and blended the images')

    # Crop background and display the stitched result
    aligned_image = crop_background_from_image(aligned_image)
    logging.info('Cropped the background')
    logging.info(f'Stitched {i + 1} images')
    display_image(aligned_image, width=20, height=10)
